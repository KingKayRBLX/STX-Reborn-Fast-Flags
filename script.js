...
document.querySelector(".save-button").addEventListener("click", () => {
  ...
  const spawnDays1 = document
    .querySelectorAll('#spawn-day-1 .day-select input[type="checkbox"]:checked')
    .map((checkbox) => checkbox.value);
  const spawnDays2 = document
    .querySelectorAll('#spawn-day-2 .day-select input[type="checkbox"]:checked')
    .map((checkbox) => checkbox.value);

  const despawnDays1 = document
    .querySelectorAll(
      '#despawn-day-1 .day-select input[type="checkbox"]:checked'
    )
    .map((checkbox) => checkbox.value);
  const despawnDays2 = document
    .querySelectorAll(
      '#despawn-day-2 .day-select input[type="checkbox"]:checked'
    )
    .map((checkbox) => checkbox.value);
